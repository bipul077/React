...
const capitalizeFirstLetter = (string)=>{
  ...
  const fetchMoreData = async () => {
    ...
    return (
      <>
            <h2 className='text-center' style={{margin:'35px 0px',marginTop:'90px'}}>24*7 News on {capitalizeFirstLetter(props.category)}</h2> 
            {loading && <Spinner/>}          
            <InfiniteScroll
              dataLength={articles.length}
              next={fetchMoreData}
              //style={{ display: 'flex', flexDirection: 'column-reverse' }} //To put endMessage and loader to the top.
              hasMore={articles.length < totalResults}//loader wont show if hasmore becomes false
              loader={<Spinner/>}
            >
